...
// Token fijo para ambiente de desarrollo - solo se usa en dev y local
...
const baseConfig = {
  ...
  USE_FIXED_TOKEN: ['dev', 'local'].includes(ENV) // Usar token fijo solo en ambiente de desarrollo y local
};
...
const envConfigs = {
  local: {
    ...baseConfig,
    USE_FIXED_TOKEN: true // Usar token fijo en ambiente local
  },
  dev: {
    ...baseConfig,
  ...
  prod: {
    ...
    // Deshabilitar toda la información de depuración en producción
    DEBUG: false
  }
};
...
export const DEBUG = config.DEBUG !== false; // Por defecto verdadero excepto en producción
...
export const getToken = () => {
  ...
  if (USE_FIXED_TOKEN) {
    if (DEBUG) console.log(`[${ENV}] Usando token fijo para desarrollo`);
    return DEV_TOKEN;
  }
...
export const updateToken = (token) => {
  if (token) {
    if (DEBUG) console.log(`[${ENV}] Actualizando token en config/env.js`);
    RUNTIME_TOKEN = token;
  }
};
